...
/**
 * Determine the base for payment-related endpoints. If the API base ends
 * with '/kg', we remove that suffix for create-payment-intent and
 * telegram-notify calls, since those routes may live at the root of
 * your backend rather than the KG router. If no API base is configured,
 * this returns an empty string so relative paths are used.
 * @param {string} url
 */
function paymentApiPath(url) {
  let base = API_BASE || '';
  // Remove '/kg' suffix when present
  if (base.endsWith('/kg')) base = base.slice(0, -3);
  return base + url;
}
...
function updateCartTotals() {
  ...
  // Tip calculation: base is subtotal + fees + delivery (only for delivery orders)
  const tipBase = subtotal + fees + (orderType === 'delivery' ? deliveryFee : 0);
  let tipValue = 0;
  if (tipPercent > 0) {
    tipValue = tipBase * tipPercent;
  } else if (customTipAmount > 0) {
    tipValue = customTipAmount;
  }
  // Round tip to 2 decimals
  tipValue = Math.max(0, Math.round(tipValue * 100) / 100);
  // Update global for later use
  // Hide tip section for pickup orders
  const tipSection = document.getElementById('tipSection');
  if (tipSection) {
    tipSection.hidden = orderType !== 'delivery';
  }
  // Update tip summary display
  const tipAmountEl = document.getElementById('tipAmount');
  if (tipAmountEl) {
    tipAmountEl.textContent = formatCurrency(tipValue);
  }
  // Add tip to total
  total += tipValue;
  document.getElementById('cartTotal').textContent = formatCurrency(total);
  // Update tip button labels with current amounts
  updateTipButtons(subtotal, fees, deliveryFee, orderType);
}

/**
 * Update the tip buttons to show dollar amounts based on the current order
 * values. Called whenever the cart totals change.
 * @param {number} subtotal
 * @param {number} fees
 * @param {number} delivery
 * @param {string} orderType
 */
function updateTipButtons(subtotal, fees, delivery, orderType) {
  const tipBase = subtotal + fees + (orderType === 'delivery' ? delivery : 0);
  document.querySelectorAll('.tip-button').forEach(btn => {
    const val = btn.getAttribute('data-tip');
    if (!val || val === 'custom') return;
    const pct = parseFloat(val);
    const amount = Math.round(tipBase * pct * 100) / 100;
    btn.textContent = `${Math.round(pct * 100)}% (${formatCurrency(amount)})`;
  });
}
...
async function initStripe() {
  ...
  // Use canMakePayment() to determine if Apple Pay / Google Pay is available.
  try {
    const prButton = elements.create('paymentRequestButton', { paymentRequest });
    const result = await paymentRequest.canMakePayment();
    if (result) {
      // Show the wallet button
      const prTarget = document.getElementById('payment-request-button');
      prTarget.hidden = false;
      prButton.mount('#payment-request-button');
    } else {
      // Hide if not available
      const prTarget = document.getElementById('payment-request-button');
      if (prTarget) prTarget.hidden = true;
    }
  } catch (err) {
    console.warn('Payment Request init failed:', err);
  }
}
...
async function createPaymentIntent() {
  ...
  // Compute base for tip (same calculation as updateCartTotals)
  const subForTip = items.reduce((s, itm) => s + itm.unitPrice * itm.quantity / 100, 0);
  const feesForTip = subForTip * 0.1;
  const baseForTip = subForTip + feesForTip + (orderType === 'delivery' ? deliveryFee : 0);
  let tipVal = 0;
  if (tipPercent > 0) {
    tipVal = baseForTip * tipPercent;
  } else if (customTipAmount > 0) {
  ...
  const payload = {
    ...
    tipCents: Math.round(tipVal * 100),
  };
  try {
    const resp = await fetch(paymentApiPath('/create-payment-intent'), {
      method: 'POST',
...
async function handleOrderSuccess(paymentIntentId) {
  ...
  try {
    fetch(paymentApiPath('/telegram-notify'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
function initEventListeners() {
  ...
  document.querySelectorAll('.tip-button').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (val === 'custom') {
        tipPercent = 0;
        // Show custom tip container
        const ctc = document.getElementById('customTipContainer');
        if (ctc) {
          ctc.classList.remove('hidden');
          ctc.setAttribute('aria-hidden', 'false');
        }
      } else {
        ...
        if (ctc) {
          ...
          ctc.setAttribute('aria-hidden', 'true');
        }
        tipPercent = parseFloat(val);
        customTipAmount = 0;
      }
      // Reset custom input when switching away
  ...
  const customInputEl = document.getElementById('customTipInput');
  if (customInputEl) {
    customInputEl.addEventListener('input', (e) => {
      ...
      tipPercent = 0;
      // Mark custom button selected and others unselected
      document.querySelectorAll('.tip-button').forEach(b => b.classList.remove('selected'));
      const customBtn = document.querySelector('.tip-button[data-tip="custom"]');
