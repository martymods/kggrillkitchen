...
// site to call the backend at www.delcotechdivision.com/kg by default. If you
// deploy your backend elsewhere, set the kg-api-base meta tag accordingly.
// Using the /kg prefix ensures CORS headers are returned from the KG router.
...
function addToCart(item) {
  ...
  if (existing) {
    ...
    if (sauceEligibleIds.has(item.id)) {
      // Default sauce is none
      cartItem.sauce = 'none';
    }
    if (freeSideEligibleIds.has(item.id)) {
      // Default free side is Jollof Rice; an empty string means none selected
...
function showMapAndDistance() {
  ...
  // Fit map to bounds with a lower maxZoom so both points are visible on mobile
