...
// API base for backend calls. Defined in index.html via window.KG_API_BASE. If not defined, defaults to empty string for same origin.
...
async function initStripe() {
  // Use the globally injected publishable key (see index.html). Do not fetch /config on the static host.
  const publishableKey = (typeof window !== 'undefined' && window.KG_STRIPE_PK) ? window.KG_STRIPE_PK : '';
  if (!publishableKey) {
...
async function createPaymentIntent() {
  ...
  const items = cart.map(item => ({ id: item.id, name: item.name, unitPrice: Math.round(item.price * 100), quantity: item.quantity }));
  const payload = {
    items,
    currency: 'usd',
    orderType,
...
async function handleOrderSuccess(paymentIntentId) {
  ...
  const order = {
    ...
    phone,
    address,
    items: cart.map(item => ({ id: item.id, name: item.name, quantity: item.quantity, unitPrice: item.price })),
    deliveryFee,
    paymentIntentId,
  };
  // Notify backend / Telegram (non‑blocking)
  try {
    // Post order details to the backend so it can notify Telegram. Do not await to avoid blocking UI.
    fetch(`${API_BASE}/telegram-notify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
