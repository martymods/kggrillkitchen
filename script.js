...
// Tip state. When delivery is selected, users can choose a tip percentage
// or custom dollar amount. currentTipPercent is a fraction (e.g. 0.15 for 15%) or null if custom. currentTipAmount is
// the computed dollar value of the tip. Both are reset when switching to pickup.
...
// Choose an API base. If none provided via meta or global, default to the
// Delco Tech KG router. Many backend routes live under this prefix, but
// payment and telegram endpoints may reside at the root of the backend.
const API_BASE = __apiBase || 'https://www.delcotechdivision.com/kg';
// Compute a base without the /kg suffix for payment/telegram calls. If
// API_BASE already lacks /kg, this will be identical to API_BASE.
const PAYMENT_BASE = API_BASE.replace(/\/kg$/, '');
/**
 * Prepend the appropriate base to a given API path. For most routes, use
 * API_BASE (which may include the /kg prefix). For payment‑related and
 * telegram routes, use PAYMENT_BASE to avoid 404s when the backend mounts
 * those handlers at the root. If a base is empty, return the path
 * untouched, allowing same‑origin calls.
 * @param {string} url an absolute path beginning with '/'
 * @returns {string} a full URL including the chosen base
 */
function api(url) {
  // Only prepend if a base is defined
  if (!url.startsWith('/')) return url;
  // For create payment or telegram calls, use PAYMENT_BASE so that
  // https://www.delcotechdivision.com/create-payment-intent works even
  // when API_BASE includes /kg.
  if (/^\/(create-payment-intent|telegram-notify)/.test(url)) {
    return PAYMENT_BASE ? `${PAYMENT_BASE}${url}` : url;
  }
...
function addToCart(item) {
  ...
  if (existing) {
    ...
    if (freeSideEligibleIds.has(item.id)) {
      // Default free side to Jollof Rice for eligible mains
      cartItem.freeSide = 'jollof_rice';
    }
    cart.push(cartItem);
  }
  ...
  cart.forEach(item => {
    ...
    row.querySelectorAll('select[data-type="freeSide"]').forEach(sel => {
      sel.addEventListener('change', (e) => {
        ...
        // Recompute totals and tip when free side changes
        updateCartTotals();
        if (typeof updateTipSection === 'function') updateTipSection();
      });
    });
    row.querySelectorAll('select[data-type="sauce"]').forEach(sel => {
      sel.addEventListener('change', (e) => {
        ...
        // Sauce change doesn't affect totals, but you could track analytics here
      });
    });
    cartContainer.appendChild(row);
  });
  updateCartTotals();
  // Update tip section after cart totals change
  if (typeof updateTipSection === 'function') {
    updateTipSection();
  }
}
...
function updateCartTotals() {
  ...
  // Tip amount (for delivery orders only). currentTipAmount will be set via
  // updateTipSection(). Add it to the total.
  if (orderType === 'delivery' && currentTipAmount > 0) {
    total += currentTipAmount;
  }
...
/**
 * Update the tip section UI and compute the current tip amount. This should
 * be called whenever the cart contents or delivery/pickup selection changes.
 */
function updateTipSection() {
  const tipSection = document.getElementById('tipSection');
  if (!tipSection) return;
  const orderType = document.querySelector('input[name="orderType"]:checked')?.value || 'pickup';
  // Hide tip section when not delivering or when cart is empty
  if (orderType !== 'delivery' || cart.length === 0) {
    tipSection.hidden = true;
    currentTipPercent = 0;
    currentTipAmount = 0;
    const tipAmtEl = document.getElementById('tipAmount');
    if (tipAmtEl) tipAmtEl.textContent = formatCurrency(0);
    return;
  }
  // Show tip UI
  tipSection.hidden = false;
  // Compute base (subtotal + fees + delivery)
  const subtotal = cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
  const fees = subtotal * 0.1;
  const base = subtotal + fees + deliveryFee;
  // Update button labels and remove active states
  const buttons = tipSection.querySelectorAll('.tip-btn');
  buttons.forEach(btn => {
  ...
  if (currentTipPercent !== null && currentTipPercent > 0) {
    currentTipAmount = base * currentTipPercent;
    // Highlight selected percentage button
    buttons.forEach(btn => {
      const val = btn.getAttribute('data-tip');
      if (parseFloat(val) === currentTipPercent) {
        btn.classList.add('active');
      }
    });
    // Hide custom input when a percent is selected
    const customContainer = document.getElementById('customTipContainer');
    if (customContainer) customContainer.hidden = true;
  } else {
    // For custom tip, show input and use its value
    const customContainer = document.getElementById('customTipContainer');
    if (customContainer) customContainer.hidden = false;
    const inputEl = document.getElementById('customTipAmount');
    const val = parseFloat(inputEl?.value) || 0;
    currentTipAmount = val;
    // Highlight custom button
    buttons.forEach(btn => {
      if (btn.getAttribute('data-tip') === 'custom') btn.classList.add('active');
    });
  }
  // Update display
  const tipAmtEl = document.getElementById('tipAmount');
  if (tipAmtEl) tipAmtEl.textContent = formatCurrency(currentTipAmount);
}
...
function updateOrderType() {
  ...
  // Always update tip section when order type changes
  if (typeof updateTipSection === 'function') {
    updateTipSection();
  }
  // Persist selection
...
async function initStripe() {
  ...
  // Check if Apple Pay / Google Pay is available. Only mount the
  // PaymentRequestButton if canMakePayment() resolves truthy.
  ...
  try {
    ...
    // If canMakePayment fails, hide the button
    document.getElementById('payment-request-button').hidden = true;
  }
}
...
async function createPaymentIntent() {
  ...
  const payload = {
    ...
    tipCents: Math.round(currentTipAmount * 100) || 0,
  };
  try {
...
function initEventListeners() {
  ...
  // Tip selection buttons and custom tip input
  const tipSectionElem = document.getElementById('tipSection');
  if (tipSectionElem) {
    // Click handlers for percentage/custom buttons
    tipSectionElem.querySelectorAll('.tip-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (val === 'custom') {
          ...
          currentTipPercent = parseFloat(val);
        }
        updateTipSection();
        updateCartTotals();
      });
    });
    // Listen for custom tip amount changes
